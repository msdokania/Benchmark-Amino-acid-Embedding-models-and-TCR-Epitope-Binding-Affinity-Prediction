...
def get_inputs(embedding_type):
    dat = pd.read_pickle("~/catELMo/Result_catELMo_embedding.pkl")
    return dat
...
def load_data_split(dat, split_type, seed):
    ...
    if split_type == "epi":
        if idx_val_fold < 0:
            ...
            idx_test = [
                index for index, pep in enumerate(x_pep) if pep in test_peptides
            ]
            ...
            # print(validation_fold_start_index, validation_fold_end_index)
            idx_test_remove_pep = idx_shuffled[
                test_fold_start_index:test_fold_end_index
            ]
            test_remove_peptides = unique_peptides[idx_test_remove_pep]
            idx_test_pep = idx_shuffled[
            ...
            idx_test_remove = [
                index for index, pep in enumerate(x_pep) if pep in test_remove_peptides
            ]
            idx_train = list(
                set(range(len(x_pep)))
                .difference(set(idx_test))
                .difference(set(idx_test_remove))
            )
    elif split_type == "tcr":
        if idx_val_fold < 0:
            ...
            validation_fold_end_index = (args.idx_val_fold + 1) * n_test
            # print(validation_fold_start_index, validation_fold_end_index)
            idx_test_remove_tcr = idx_shuffled[
                test_fold_start_index:test_fold_end_index
            ]
            test_remove_tcrs = unique_tcrs[idx_test_remove_tcr]
            idx_test_tcr = idx_shuffled[
                validation_fold_start_index:validation_fold_end_index
            ]
            test_tcrs = unique_tcrs[idx_test_tcr]
            idx_test = [index for index, tcr in enumerate(x_tcr) if tcr in test_tcrs]
            idx_test_remove = [
                index for index, tcr in enumerate(x_tcr) if tcr in test_remove_tcrs
            ]
            idx_train = list(
                set(range(len(x_tcr)))
                .difference(set(idx_test))
    ...
    print(
        "number of overlapping tcrs: ",
        str(len(set(trainData.tcr).intersection(set(testData.tcr)))),
    )
    print(
        "number of overlapping epitopes: ",
        str(len(set(trainData.epi).intersection(set(testData.epi)))),
    )

    # tcr_split testing read
    X1_test_list, X2_test_list, y_test_list = (
        testData.tcr_embeds.to_list(),
        testData.epi_embeds.to_list(),
        testData.binding.to_list(),
    )
    X1_test, X2_test, y_test = (
        np.array(X1_test_list),
        np.array(X2_test_list),
        np.array(y_test_list),
    )
    # tcr_split training read
    X1_train_list, X2_train_list, y_train_list = (
        trainData.tcr_embeds.to_list(),
        trainData.epi_embeds.to_list(),
        trainData.binding.to_list(),
    )
    X1_train, X2_train, y_train = (
        np.array(X1_train_list),
        np.array(X2_train_list),
...
def train_(embedding_name, X1_train, X2_train, y_train, X1_test, X2_test, y_test):
    ...
    # ## model fit
    # checkpoint_filepath = embedding_name + ".hdf5"
    ...
    print("Inside the train function now ------------------")
    ...
    #if embedding_name.find("tcr")!=-1:
     #   dump(model, 'models/trained_RF_catelmo_tcr.joblib')
    #else:
     #   dump(model, 'models/trained_RF_catelmo_epi.joblib')
    

    # ==================== UNCOMMENT THIS TO USE PRE-TRAINED SAVED MODEL=======================
    if embedding_name.find("tcr")!=-1:
        model = load('models/trained_RF_catelmo_tcr.joblib')
    else:
        model = load('models/trained_RF_catelmo_epi.joblib')
    ...
    precision1 = precision_score(y_test, yhat, pos_label=1, zero_division=0)
    precision0 = precision_score(y_test, yhat, pos_label=0, zero_division=0)
    ...
    print(
        "precision_recall_fscore_macro "
        + str(precision_recall_fscore_support(y_test, yhat, average="macro"))
    )
...
def main(embedding, split, fraction, seed, gpu):
    ...
    (
        ...
        testData,
        trainData,
    ) = load_data_split(tr_dat, split, seed)
    train_(
        embedding + "_" + split + "_seed_" + str(seed) + "_fraction_" + str(fraction),
        X1_train,
        X2_train,
        y_train,
        X1_test,
