...
def get_inputs(embedding_type):
    dat = pd.read_pickle("~/catELMo/Result_SeqVec_embedding.pkl")
    return dat
...
def train_(embedding_name, X1_train, X2_train, y_train, X1_test, X2_test, y_test):
    ...
    #model.fit(X_train, y_train)
     ...
     #   dump(model, 'models/trained_RF_seqvec_tcr.joblib')
    #else:
     #   dump(model, 'models/trained_RF_seqvec_epi.joblib')

# ==================== UNCOMMENT THIS TO USE PRE-TRAINED SAVED MODEL=======================
    if embedding_name.find("tcr")!=-1:
        model = load('models/trained_RF_seqvec_tcr.joblib')
    else:
        model = load('models/trained_RF_seqvec_epi.joblib')
